...
class dropout(object):
    ...
    def get_chip_samples(self, sample_sheet, chip=1):
        ...
        sample_sheet = sample_sheet[sample_sheet['Chip #'] == chip]
...
"""%self.STYLE_CSS
        ...
        dropout_files = os.listdir(os.path.join(self.DROPOUT_DIR,run_id))
        anno_amp_df_list = list()
        for chip, one_file in enumerate(dropout_files):
            dropout = pd.read_table(os.path.join(self.DROPOUT_DIR,run_id,one_file))
            anno_amp_df = self.process_dropout(dropout, chip+1)
